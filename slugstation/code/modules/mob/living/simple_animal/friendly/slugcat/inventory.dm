...
/mob/living/simple_animal/slugcat/get_item_by_slot(slot_id)
	switch(slot_id)
		if(SLOT_HEAD)
			return head
		if(SLOT_GENERC_DEXTROUS_STORAGE)
...
/mob/living/simple_animal/slugcat/equip_to_slot(obj/item/I, slot)
	if(!slot)
		return
	if(!istype(I))
		return
...
/mob/living/simple_animal/slugcat/getBeltSlot()
	return SLOT_GENERC_DEXTROUS_STORAGE
